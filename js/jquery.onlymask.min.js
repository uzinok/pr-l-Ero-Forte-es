 ...
 * Date: 14-07-2021
 */
;(function($){$.fn.onlymask = function(options) {var codeRegex = /^[a-z]{2}$/i; var formatRegex = /^(\d+)([\s\-\(\)\*\{\}\,\d]+)$/; var defaults = {sendCode: true, displayCode: true, displayLeadingPlus: true, formats: {default: "[0-9]*", ru: "7**********"} }; var opts = $.extend(true, defaults, options); var maskOptionsDefault = {showMaskOnHover: false }; function codeExists(code) {return code in opts.formats; } function codeLength(str) {var result = 0; while( /\+|\d/.test(str.charAt(0)) ) {str = str.substr(1); result++; } return result; } function getPartFormat(part, format) {switch(part) {case "phone": if (!opts.displayCode) {return format.substr(codeLength(format)); } break; case "code": return format.substr(0, codeLength(format)); break; } return format; } function getPluginValue(pluginOption, country) {switch(pluginOption) {case "mask": return getPartFormat("phone", opts.formats[country]).replace(/9/g, "\\9").replace(/\*/g, "9"); break; case "placeholder": return getPartFormat("phone", opts.formats[country]); break; } return ""; } function maskOptions(country) {var result = {}; if ( !codeExists(country) ) {console.warn("There is no code for the selected country: " + country.toUpperCase()); result.regex = opts.formats["default"]; result.placeholder = ""; } else {result.mask = getPluginValue("mask", country); result.placeholder = getPluginValue("placeholder", country); } return result; } function prepareMarkup() {$('[name="phone"]').wrap($('<div>') .addClass('onlymask') .css('position', 'relative') ); $('[name="phone"]').each(function(){var $parent = $(this).closest('.onlymask'); $(this) .clone() .appendTo( $parent ) .removeAttr('name') .addClass('onlymask__input--visible') .attr('style', 'position: relative; z-index: 10'); $(this) .prop('readonly', true) .addClass('onlymask__input--hidden') .attr('style', 'position: absolute; z-index: 1; top: 0;') }); $('[name="phone"]').on('focus', function(){var $mask = $(this).closest('.onlymask').find('.onlymask__input--visible'); $mask.focus(); }); $('.onlymask__input--visible').on('input', function(){var $this = $(this); var $phone = $this.closest('.onlymask').find('[name="phone"]'); var country = $this.closest("form").find("#country_code_selector").val().toLowerCase(); var phoneVal = $this.inputmask('unmaskedvalue'); if ( opts.sendCode && codeExists(country) ) {phoneVal = getPartFormat("code", opts.formats[country]) + phoneVal; } $phone.val(phoneVal); setTimeout(function(){if ( !$this.val() || !$this.inputmask("isComplete") ) {$phone.val(''); } }, 50); }); } function initMask($number, $country) {var selected = $country.val() && $country.val().toLowerCase(); if ( !selected ) {throw new Error("There is no options for select element"); } $number.inputmask($.extend({}, maskOptionsDefault, maskOptions(selected))); } function handleForms() {$('form').each(function() {var $form = $(this); var $country = $(this).find("#country_code_selector"); var $number = $(this).find('.onlymask__input--visible').length ? $(this).find('.onlymask__input--visible') : $(this).find('.only_number'); $country.on('change', function() {if ( typeof $number.inputmask !== undefined) {$number.inputmask('setvalue', ''); $form.find('[name="phone"]').val(''); $number.inputmask('remove', ''); } initMask($number, $(this)); }); initMask($number, $country); }); } function init() {for (key in opts.formats) {if ( key === "default" ) {continue ; } if ( !codeRegex.test(key) || !formatRegex.test(opts.formats[key]) ) {throw new Error(['"'+key.toUpperCase()+'"', 'format is invalid'].join(" ")); } if (opts.displayLeadingPlus) {opts.formats[key] = "+" + opts.formats[key]; } } prepareMarkup(); handleForms(); } init(); }; })(jQuery);